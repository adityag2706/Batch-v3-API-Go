...
This script uses a JSON file "batch_request.json".
...
func main() {
	...
	// Default filename is batch_request.json in the same location as the go code.
	fileName := flag.String("file", "batch_request.json", "filename for batch request payload")
	flag.Parse()
